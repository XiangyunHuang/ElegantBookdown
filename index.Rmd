...
3. 不要自定义 Pandoc's LaTeX 模版，Pandoc 内建的模版已经功能很全面了，下游的 R Markdown 生态已经甩掉了自己造的大量 LaTeX 模版。为保持与下游的完美兼容，也为了更加轻量地输出多种文档格式，尽可能多种输出格式的风格一致。
4. 本书输出格式目标是 PDF/HTML/EPUB，可以推动上游优化 Pandoc 模版或者 [ElegantBook](https://github.com/ElegantLaTeX/ElegantBook) 模版
...
## 用 Pandoc 生成定理环境  {#pandocDiv}

以下各种定理环境所用css可以在[thmSytle.css](/thmStyle.css)中找到.
...
:::{.defn}
A function is a linear function if its graph is a line.
:::

:::{.defn}
Let $f$ be a function defined in an interval containing $a$. We say that $f$ is continuous at $a$ if $\lim\limits_{x\to a} f(x)=f(a)$.
:::
...
:::{.exer name="Characteristic Function of the Sample Mean"}
...
数学公式加粗可能是最常见的需求之一，所以 **elegantbook** 宏包提供的文类 `elegantbook.cls` 已经调用了 **bm** 宏包 [^load-bm]。有了 **bm** 宏包，就可以使用 **bm** 宏包提供的 `\bm{}` 命令，而不需要写 `\boldsymbol{}` 来加粗希腊字母，如将  $\alpha$ （正常）加粗为 $\bm{\alpha}$（粗体）。要想在 HTML 网页中显示加粗效果，则还不够，默认情况下， MathJax 是不认识 `\bm{}` 命令的，所以需要在 `header.html` 自定义
...
[^custom-block]: 截止 `r format(Sys.time(), '%d %B, %Y')` 该书尚未出版，自定义 block 的章节见 <https://bookdown.org/yihui/rmarkdown-cookbook/custom-blocks.html>
